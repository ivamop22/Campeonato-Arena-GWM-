...
import { Trophy, Users, Zap, RotateCcw, Play, Check, CheckCircle2, XCircle } from 'lucide-react';
...
export default function TournamentApp({ tournamentId, tournamentName }: Props) {
  ...
  // Maps local player index → Supabase UUID
  const [playerUuids, setPlayerUuids] = useState<string[]>([]);
  // Maps local match index → Supabase UUID
  ...
  const loadData = useCallback(async () => {
    const [{ data: dbPlayers }, { data: dbMatches }] = await Promise.all([
      ...
      supabase.from('tournament_matches').select('*').eq('tournament_id', tournamentId).order('match_index'),
    ]);

    if (dbPlayers && dbPlayers.length > 0) {
      const localPlayers: Player[] = dbPlayers.map((p, i) => ({ id: i, name: p.name }));
      const uuids = dbPlayers.map((p) => p.id);
      setPlayers(localPlayers);
      setPlayerCount(localPlayers.length);
      setPlayerUuids(uuids);

      if (dbMatches && dbMatches.length > 0) {
        const localMatches: Match[] = dbMatches.map((m) => {
          const t1p1 = uuids.indexOf(m.team1_player1_id);
          const t1p2 = uuids.indexOf(m.team1_player2_id);
          const t2p1 = uuids.indexOf(m.team2_player1_id);
          const t2p2 = uuids.indexOf(m.team2_player2_id);
          return {
            id: m.match_index,
            team1: [t1p1, t1p2] as [number, number],
            team2: [t2p1, t2p2] as [number, number],
            score1: m.score1,
            score2: m.score2,
          };
        });
        setMatches(localMatches);
        setMatchUuids(dbMatches.map((m) => m.id));
        setGameStarted(true);
      }
    }
  }, [tournamentId]);
  ...
  const generateMatches = async () => {
    ...
    if (!schedule) {
      toast.error('Não foi possível gerar torneio válido.');
      return;
    }

    // Save players to Supabase first (delete old ones)
    ...
    const { data: savedPlayers, error: playersError } = await supabase
      ...
      .insert(players.map((p) => ({ tournament_id: tournamentId, name: p.name })))
      .select('id');

    if (playersError || !savedPlayers) {
      toast.error('Erro ao salvar jogadores.');
      return;
    }

    const uuids = savedPlayers.map((p) => p.id);
    setPlayerUuids(uuids);

    // Save matches to Supabase
    await supabase.from('tournament_matches').delete().eq('tournament_id', tournamentId);
    const matchRows = schedule.map((g, idx) => ({
      tournament_id: tournamentId,
      match_index: idx,
      team1_player1_id: uuids[g.t1[0]],
      team1_player2_id: uuids[g.t1[1]],
      team2_player1_id: uuids[g.t2[0]],
      team2_player2_id: uuids[g.t2[1]],
      score1: null,
      score2: null,
    }));
    ...
    if (matchesError || !savedMatches) {
      toast.error('Erro ao salvar partidas.');
      return;
    }

    setMatchUuids(savedMatches.map((m) => m.id));
    ...
    setGameStarted(true);
    toast.success('Chaveamento gerado e salvo!');
  };

  const updateScore = async (matchId: number, team: 1 | 2, delta: number) => {
    let newScore1: number | null = null;
    let newScore2: number | null = null;
    ...
    setMatches((prev) =>
      prev.map((m) => {
        ...
        if (team === 1) {
          ...
          newScore1 = Math.max(0, Math.min(4, current + delta));
          return { ...m, score1: newScore1 };
        } else {
          const current = m.score2 ?? 0;
          newScore2 = Math.max(0, Math.min(4, current + delta));
          return { ...m, score2: newScore2 };
        }
      })
    );

    const matchUuid = matchUuids[matchId];
    if (matchUuid) {
      const updateData = team === 1 ? { score1: newScore1 } : { score2: newScore2 };
      await supabase.from('tournament_matches').update(updateData).eq('id', matchUuid);
    }
  };
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 py-8 md:py-12">
        ...
        >
          ...
          <h1 className="text-4xl md:text-6xl font-bold tracking-tight mb-2">
            <span className="text-primary">{tournamentName}</span>
          </h1>
